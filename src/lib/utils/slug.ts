 ...
 * @example
 * await generateUniqueSlug("Pikachu VMAX", "item") // "pikachu-vmax"
 * await generateUniqueSlug("Pikachu VMAX", "item") // "pikachu-vmax-2" (if first exists)
 */
export async function generateUniqueSlug(
  ...
  while (true) {
    const conditions = excludeId ? [eq(table.id, excludeId)] : []

    const [result] = await db
      .select({ count: count() })
      .from(table)
      .where(
        sql`${table.slug} = ${slug}${conditions.length > 0 ? sql` AND ${table.id} != ${excludeId}` : sql``}`,
      )
