...
<div class="flex-1 space-y-4">
  ...
  <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
    <Card>
      ...
      <CardContent>
        ...
        <p class="text-xs text-muted-foreground">From {data.stats.totalOrders} orders</p>
      </CardContent>
    </Card>
    ...
    <Card>
      ...
      <CardContent>
        <div class="text-2xl font-bold">{data.stats.pendingOrders}</div>
    ...
    <Card>
      ...
      <CardContent>
        <div class="text-2xl font-bold">{data.stats.totalItems}</div>
        <p class="text-xs text-muted-foreground">
          In {data.stats.totalCategories} categories
        </p>
      </CardContent>
    </Card>
    ...
    <Card>
      ...
      <CardContent>
        <div class="text-2xl font-bold text-yellow-600">{data.stats.lowStockItems}</div>
        <p class="text-xs text-muted-foreground">
          <span class="text-destructive">{data.stats.outOfStockItems} out of stock</span>
        </p>
      </CardContent>
    </Card>
  </div>
  ...
  <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
    ...
    <Card class="col-span-3">
      ...
      {#if data.stats.outOfStockItems > 0 || data.stats.lowStockItems > 0}
        <CardHeader class="pt-0">
          <CardTitle class="text-sm">Inventory Alerts</CardTitle>
        </CardHeader>
        <CardContent class="space-y-2">
          {#if data.stats.outOfStockItems > 0}
            <div class="flex items-center gap-2 text-sm">
              <PackageXIcon class="h-4 w-4 text-destructive" />
              <span class="font-medium text-destructive">
                {data.stats.outOfStockItems} item{data.stats.outOfStockItems !== 1 ? "s" : ""} out of
                stock
              </span>
            </div>
          {/if}
          {#if data.stats.lowStockItems > 0}
            <div class="flex items-center gap-2 text-sm">
              ...
              <span class="font-medium text-yellow-600">
                {data.stats.lowStockItems} item{data.stats.lowStockItems !== 1 ? "s" : ""} low on stock
              </span>
            </div>
          {/if}
        </CardContent>
