...
<div class="container mx-auto space-y-6 px-4 py-6 md:px-6 md:py-8 lg:px-8 xl:px-12">
  <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div class="flex flex-1 items-center gap-2">
      <div class="relative max-w-sm flex-1">
        ...
        <Input type="search" placeholder="Search cards..." class="pl-8" bind:value={searchQuery} />
      </div>
    </div>
  ...
  <div class="pb-4 text-center text-sm text-muted-foreground">
    Showing {filteredItems.length} of {data.items.length} item{data.items.length !== 1 ? "s" : ""}
  </div>
</div>
